...
qb_load_env_file_defaults() {
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    value="${line#*=}"
    case "${key}" in
      PORT|ENV|SEED_MODE|SETUP_BOOTSTRAP|FRONTEND_DIR|UPLOAD_DIR|BACKUP_DIR|DB_DSN|JWT_SECRET|JWT_REFRESH_SECRET|BASE_URL|CORS_ALLOWED_ORIGINS|PLUGIN_DIR|PLUGIN_DATA_DIR|ENABLE_EXTERNAL_PLUGINS)
        if [[ -z "${!key+x}" ]]; then
