...
**Author:** Inkless project
**Date:** 2026-06-14
**Reference consumer:** [inkless](https://github.com/yixian-huang/inkless.git) (`scripts/build-*.sh`, `scripts/deploy.sh`, `scripts/rollback.sh`)
...
```json
{
  "repoUrl": "https://github.com/yixian-huang/inkless.git",
  "environmentName": "hk",
  ...
  "envVars": {
    "PORT": { "value": "8088" },
    "FRONTEND_DIR": { "value": "/opt/inkless/frontend/current" },
    "DB_DSN": { "value": "file:/opt/inkless/data/inkless.db?cache=shared&mode=rwc", "isSecret": false },
...
### 4.3 心跳与僵死检测（P0，吸取 inkless 部署教训）
...
BACKUP_DIR=/opt/inkless/backups
BASE_URL=https://www.example.com
CORS_ALLOWED_ORIGINS=https://www.example.com,https://admin.example.com
PLUGIN_DIR=/opt/inkless/plugins
PLUGIN_DATA_DIR=/opt/inkless/data/plugins
  ...
  "components": [
    {
      ...
      "runtime": {
        "binaryName": "inkless-api-{version}",
        "symlink": "backend/current/inkless-api-latest"
      }
    },
    {
      "name": "frontend",
...
## 7. inkless `scripts/` 对接约定（参考实现）

QB team 可将下列脚本作为 **conformance test** 的消费者；inkless 侧在 artifact 支持落地后新增。
...
3. 原子切换 `current` / `inkless-api-latest` 软链
...
`activate` 后二进制从 `/opt/inkless/backend/current/inkless-api-latest` 启动（或 systemd `ExecStart` 指向该路径）。
...
| 参数 | 实例 A | 实例 B |
| --- | --- | --- |
| `QB_RELEASE_ROOT` | `/opt/inkless-a` | `/opt/inkless-b` |
| `QB_SYSTEMD_UNIT` | `inkless-a` | `inkless-b` |
| `PORT` | `8088` | `8089` |
| `DB_DSN` | `file:/opt/inkless-a/data/inkless.db?cache=shared&mode=rwc` | `file:/opt/inkless-b/data/inkless.db?cache=shared&mode=rwc` |
| `UPLOAD_DIR` | `/opt/inkless-a/uploads` | `/opt/inkless-b/uploads` |
| `BACKUP_DIR` | `/opt/inkless-a/backups` | `/opt/inkless-b/backups` |
| `PLUGIN_DIR` | `/opt/inkless-a/plugins` | `/opt/inkless-b/plugins` |
| `PLUGIN_DATA_DIR` | `/opt/inkless-a/data/plugins` | `/opt/inkless-b/data/plugins` |
...
`ExecStart` 必须分别指向 `/opt/inkless-a` 与 `/opt/inkless-b`；在
`ProtectSystem=strict` 下，`ReadWritePaths` 还必须放行各自的备份、插件包和插件数据目录。
activate 脚本会在 `${QB_SYSTEMD_UNIT}.service.d/inkless-instance-paths.conf` 写入对应
...
另一个实例。端口被占用时可设置 `INKLESS_SMOKE_PORT_A`、`INKLESS_SMOKE_PORT_B`；传入
`INKLESS_SMOKE_BINARY` 可复用已构建的 server 二进制。
...
| inkless `hk` 生产 | **`artifact`**（build server 编译 → hk 仅激活，避免现场 docker build） |

---

## 11. 验收标准（以 inkless 为 golden path）
...
**文档版本:** 0.1.0-draft  
**下一步:** inkless 仓库新增 `scripts/qb-artifact-*.sh` 参考实现；QB team 评审 API 字段与状态机后迭代 0.2。
