...
One Inkless instance serves one logical site. `BASE_URL` is its sole canonical origin;
...
```bash
# Site A
QB_RELEASE_ROOT=/opt/inkless-a QB_SYSTEMD_UNIT=inkless-a PORT=8088 \
DB_DSN='file:/opt/inkless-a/data/inkless.db?cache=shared&mode=rwc' \
UPLOAD_DIR=/opt/inkless-a/uploads PLUGIN_DIR=/opt/inkless-a/plugins \
BACKUP_DIR=/opt/inkless-a/backups PLUGIN_DATA_DIR=/opt/inkless-a/data/plugins \
BASE_URL=https://a.example.com \
CORS_ALLOWED_ORIGINS=https://a.example.com ./scripts/qb-artifact-activate.sh

# Site B uses /opt/inkless-b, inkless-b, port 8089, its own directories and secrets.
```
